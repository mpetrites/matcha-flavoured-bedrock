...
python3 tools/check_full_parity.py --source /path/to/unpacked/Matcha_Flavoured_1_03
```
...
`check_full_parity.py` additionally inventories every file under `data/` and
`assets/`; it fails if an upstream namespace/type has no explicit converted,
implemented, classified, or engine-limited owner.
...
`build.py` is the public entry point for the complete build. It runs
all recipe and item generation in dependency order, converts the remaining
pack surfaces, runs every audit, and packages the add-on only after all checks
pass:

```sh
python3 tools/build.py --source /path/to/unpacked/Matcha_Flavoured
```

The structure stage requires `nbtlib` in the build environment. A local,
non-project install keeps this dependency out of the add-on:

```sh
python3 -m pip install --target /tmp/matcha_pydeps nbtlib
PYTHONPATH=/tmp/matcha_pydeps python3 tools/build.py \
  --source /path/to/unpacked/Matcha_Flavoured
```

The internal `convert_foods.py` stage generates custom items, recipe variants,
texture-atlas entries, names, and
...
Equipment generation itself is run through `build.py`.
...
Singleton vanilla carriers are generated by `build.py` and checked
separately. Vanilla bases with multiple Matcha forms are deliberately excluded:
...
with valid Bedrock IDs or crafting fallbacks. Colored-banner recipes use their
valid Bedrock banner identifiers. Custom music discs use the native events `11` and
